      ...
      <div class="wrap">
        <div class="contact-grid">
          <div class="contact-card">
            <h3>Get In Touch</h3>
            <p style="margin-top:12px"><strong>Email</strong><br><a href="mailto:codyzippe1@gmail.com">codyzippe1@gmail.com</a></p>
...
cart_extra = r'''  <script>
    ...
    function renderPayment() {
      ...
      $('step3').innerHTML = `
        <div class="panel pay-panel">
          ...
          <button type="button" class="btn" data-go="4">I've sent the payment</button>
        </div>`;
    }

    function renderDone() {
      const o = S.order;
      if (!S.sent) { S.sent = true; CK.save(); sendOrder(o); Cart.clear(); }
      $('step4').innerHTML = `
        <div class="panel done-panel">
          ...
          <p>We'll confirm your ${payName(S.pay)} payment and email a FedEx tracking number to <strong>${esc(o.email)}</strong> as soon as your order ships.</p>
          <p class="muted">Keep your order number handy. Questions? <a href="mailto:${STORE.orderEmail}">${STORE.orderEmail}</a></p>
    ...
    function renderSummary(t) {
      ...
      $('sumActions').innerHTML = {
        ...
        3: `<button type="button" class="btn" data-go="4">I've sent the payment</button><button type="button" class="btn outline" data-go="2">Back to Shipping</button>`,
        4: `<a class="btn" href="peptides.html">Continue Shopping</a>`
      }[S.step];
    }

    // ---------- Send the order to the owner ----------
    function orderLines(o) {
      return [
        'NEW ORDER ' + o.orderNumber, '',
        ...o.items.map(i => `${i.qty} x ${i.name} (${i.sku}) @ ${money(i.price)} = ${money(i.lineTotal)}`), '',
        `Boxes: ${o.boxes}`, `Subtotal: ${money(o.subtotal)}`,
        `Shipping: ${o.shippingMethod} ${o.shipping === 0 ? 'FREE' : money(o.shipping)}`,
        `TOTAL: ${money(o.total)}`, `Payment: ${o.paymentMethod}`, '',
        `Name: ${o.name}`, `Address: ${o.address}`, `Email: ${o.email}`, `Phone: ${o.phone}`
      ];
    }
    function mailto(o) {
      return 'mailto:' + STORE.orderEmail + '?subject=' + encodeURIComponent('Proper Peptides order ' + o.orderNumber) + '&body=' + encodeURIComponent(orderLines(o).join('\n'));
    }
    function sendOrder(o) {
      const usingFormspree = STORE.formspree && !STORE.formspree.includes('YOUR_FORM_ID');
      if (!usingFormspree) { window.location.href = mailto(o); return; }
      fetch(STORE.formspree, {
        method: 'POST', headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
        body: JSON.stringify({ ...o, _subject: 'Proper Peptides order ' + o.orderNumber, _replyto: o.email, summary: orderLines(o).join('\n') })
      }).then(r => { if (!r.ok) throw new Error(); }).catch(() => { window.location.href = mailto(o); });
    }
    ...
    $('shipForm').addEventListener('submit', e => {
      ...
      S.shipId = d.get('ship'); S.pay = d.get('pay');
      goto(3);
    });
